...
class TableRow:
    ...
    def show(self, striped=False):
        ...
        if self._iid is None:
            self.build()
        self.view.reattach(self.iid, "", tk.END)
    ...
    def build(self):
        ...
        """
        if self._iid is None:
            self._iid = self.view.insert("", tk.END, values=self.values)
...
class Tableview(ttk.Frame):
    ...
    def __init__(
        ...
        self._build_tableview_widget(coldata, rowdata)
    ...
    def insert_row(self, index=tk.END, values=[]) -> TableRow:
    ...
    def insert_rows(self, index, rowdata, sort: int = 0):
    ...
    def get_column(self, index=None, visible=False, cid=None) -> TableColumn:
    ...
    def autofit_columns(self):
        ...
        for col in self.tablecolumns:
            width = f.measure(f"{col._headertext} {DOW_NARROW}") + pad
    ...
    def autoalign_columns(self):
        ...
        for i, value in enumerate(values):
            if str(value).isnumeric():
                self.view.column(i, anchor=tk.E)
                self.view.heading(i, anchor=tk.E)
            else:
                self.view.column(i, anchor=tk.W)
                self.view.heading(i, anchor=tk.W)
    ...
    def align_column_left(self, event=None, cid=None):
        ...
        if event is not None:
            eo = self._get_event_objects(event)
            self.view.column(eo.column.cid, anchor=tk.W)
        elif cid is not None:
            self.view.column(cid, anchor=tk.W)
    ...
    def align_column_right(self, event=None, cid=None):
        ...
        if event is not None:
            eo = self._get_event_objects(event)
            self.view.column(eo.column.cid, anchor=tk.E)
        elif cid is not None:
            self.view.column(cid, anchor=tk.E)
    ...
    def align_column_center(self, event=None, cid=None):
        ...
        if event is not None:
            eo = self._get_event_objects(event)
            self.view.column(eo.column.cid, anchor=tk.CENTER)
        elif cid is not None:
            self.view.column(cid, anchor=tk.CENTER)
    ...
    def align_heading_left(self, event=None, cid=None):
        ...
        if event is not None:
            eo = self._get_event_objects(event)
            self.view.heading(eo.column.cid, anchor=tk.W)
        elif cid is not None:
            self.view.heading(cid, anchor=tk.W)
    ...
    def align_heading_right(self, event=None, cid=None):
        ...
        if event is not None:
            eo = self._get_event_objects(event)
            self.view.heading(eo.column.cid, anchor=tk.E)
        elif cid is not None:
            self.view.heading(cid, anchor=tk.E)
    ...
    def align_heading_center(self, event=None, cid=None):
        ...
        if event is not None:
            eo = self._get_event_objects(event)
            self.view.heading(eo.column.cid, anchor=tk.CENTER)
        elif cid is not None:
            self.view.heading(cid, anchor=tk.CENTER)
    ...
    def _column_sort_header_update(self, cid):
        ...
        column: TableColumn = self.cidmap.get(int(cid))
        arrow = UP_ARROW if column.columnsort == ASCENDING else DOW_NARROW
        headertext = f"{column.headertext} {arrow}"
        self.view.heading(column.cid, text=headertext)

    # PRIVATE METHODS - WIDGET BUILDERS

    def _build_tableview_widget(self, coldata, rowdata):
        """Build the data table"""
        if self._searchable:
            self._build_search_frame()

        tree_frame = ttk.Frame(master=self, padding=2)
        tree_frame.pack(fill=tk.BOTH, expand=tk.YES, side=tk.TOP)
        ...
        self.vbar = ttk.Scrollbar(
            master=tree_frame, command=self.view.yview, orient="vertical"
        )
        self.vbar.pack(fill=tk.Y, side=tk.RIGHT)
        self.hbar = ttk.Scrollbar(
            master=tree_frame, command=self.view.xview, orient="horizontal"
        )
        self.hbar.pack(fill=tk.X, side=tk.BOTTOM)
        self.view.configure(xscrollcommand=self.hbar.set)
        self.view.configure(yscrollcommand=self.vbar.set)
        self.view.pack(fill=tk.BOTH, expand=tk.YES, side=tk.LEFT)
    ...
    def _build_search_frame(self):
        ...
        searchterm.pack(fill=tk.X, side=tk.LEFT, expand=tk.YES)
    ...
    def _build_pagination_frame(self):
        ...
        pageframe.pack(fill=tk.X, anchor=tk.N)
        ...
        ttk.Separator(pageframe, orient="vertical").pack(side=tk.RIGHT, padx=10)
        ...
        lbl.pack(side=tk.RIGHT, padx=(0, 5))
        ttk.Label(pageframe, text="of ").pack(side=tk.RIGHT, padx=(5, 0))

        index = ttk.Entry(pageframe, textvariable=self._pageindex, width=4)
        index.pack(side=tk.RIGHT)
        index.bind("<Return>", self.goto_page, "+")
        index.bind("<KP_Enter>", self.goto_page, "+")

        ttk.Label(pageframe, text="Page").pack(side=tk.RIGHT, padx=5)
    ...
    def _build_table_columns(self, coldata):
        ...
        for cid, col in enumerate(coldata):
            if isinstance(col, str):
                ...
                self.tablecolumns.append(TableColumn(tableview=self, cid=cid, **col))
...
#         config = {
#             "sortascending": {
#                 "label": f"""⬆  {"Sort Ascending"}""",
#                 "command": self.sort_column_ascending,
#             },
#             "sortdescending": {
#                 "label": f"""⬇  {"Sort Descending"}""",
...
#             },
#             "filterbyvalue": {
#                 "label": f"""{"Filter by cell's value"}""",
#                 "command": self.filter_to_cell_value,
#             },
#             "hiderows": {
#                 "label": f"""{"Hide select rows"}""",
...
#         self.add_cascade(
#             menu=sort_menu, label=f"""⇅  {MessageCatalog.translate("Sort")}"""
#         )
...
#         self.add_cascade(
#             menu=filter_menu, label=f"""⧨  {MessageCatalog.translate("Filter")}"""
#         )
...
#         self.add_cascade(
#             menu=export_menu, label=f"""↔  {MessageCatalog.translate("Export")}"""
#         )
...
#         self.add_cascade(
#             menu=move_menu, label=f"""⇵  {MessageCatalog.translate("Move")}"""
#         )
...
#         self.add_cascade(
#             menu=align_menu, label=f"""↦  {MessageCatalog.translate("Align")}"""
#         )
...
#         self.add_cascade(
#             menu=self._show_menu, label=f"""±  {MessageCatalog.translate("Columns")}"""
#         )
#         self.add_separator()
...
#             self._show_menu.delete(0, tk.END)
#         else:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        Parameters:
            ...
            autohide (bool):
                When **True**, the scrollbar will hide when the mouse
        ...
        self.vscroll = ttk.Scrollbar(
            master=self.container, command=self.yview, orient=tk.VERTICAL
        )
